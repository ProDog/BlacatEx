...
namespace NFT_API
{
    class ZoroHelper
    {
        private static string api = Config.getStrValue("myApi");
        ...
        {
            string url = api + "?jsonrpc=2.0&id=1&method=getblockcount&params=['']";
            var result = Helper.HttpGet(url);
            return int.Parse(JObject.Parse(result)["result"].ToString());
        }
